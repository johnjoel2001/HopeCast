...
from scripts.rag.rag import RAGSystem  # Importing RAGSystem from scripts/rag/rag.py
import numpy as np
...
def format_label(column: str) -> str:
    """
    Formatting column name by replacing underscores with spaces and capitalizing words.
...
class DatasetLoader:
    ...
    """
    Handling loading and validation of clinical, genetic, and mutation datasets
    
    """
    
    def __init__(self, data_dir: str = "data"):
        """
        Initializing DatasetLoader with directory path.
...
class SurvivalModel:
    ...
    def __init__(self, model_dir: str = "models/deep_learning"):
        """
        Initializing SurvivalModel with model directory
    ...
    def _load_model(self, dataset_type: str) -> tuple:
        """
        Loading pre-trained CoxPH model, scaler, and feature names
        ...
        try:
            ...
            # Importing PyTorch-related modules here to avoid Streamlit file watcher issues
            import torch
            ...
            # Setting learning rate in optimizer to avoid 'lr' error
    ...
    def _preprocess_data(self, df: pd.DataFrame, user_input: dict, feature_names: list) -> tuple:
        """
        Preprocessing dataset and user input for prediction
        ...
        """
        # Importing numpy here to avoid file watcher issues

        import numpy as np

        # Handling data imbalance by upsampling severe cases
        ...
        # Aligning encoded dataset with model feature names
        ...
        # Aligning user input with model feature names
        for col in feature_names:
    ...
    def predict(self, df: pd.DataFrame, user_input: dict, dataset_type: str) -> tuple:
        """
        Predicting median survival time using CoxPH model and return survival curve
...
class StreamlitApp:
    ...
    def __init__(self):
        ...
        """
        Initializing StreamlitApp with dataset loader, model, and RAG system
        ...
        # Applying custom CSS for UI styling
        st.markdown("""
            <style>
            .main {
    ...
    def _create_input_fields(self, dataset_type: str) -> dict:
        ...
        # Grouping  key fields in an expander
        with st.expander("Key Patient Information", expanded=True):
            ...
            for i, col in enumerate(feature_cols):
                ...
                # Assigning field to left or right column
                with cols[i % 2]:
    ...
    def run(self):
        ...
        if st.button(" Predict & Explain"):
            with st.spinner("Processing patient data..."):
                try:
                    # Predicting survival time and get survival curve
                    df = self.dataset_loader.get_dataset(dataset_type)
                    expected_time, surv_input = self.model.predict(df, user_input, dataset_type)

                    # Displaying survival curve at specific intervals (1, 5, 10, 15, 20 months)
                    ...
                    for desired_time in desired_times:
                        ...
                        # Finding the closest time point in the survival curve to the desired time
                    ...
                    # Displaying median survival time with nice wording
